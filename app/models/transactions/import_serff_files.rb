  ...
  class ImportSerffFiles
    ...
    def process_service_area(input)
      ...
      begin
        ...
        return Failure({errors: ["Failed to store data from file #{File.basename(sa_params[:sa_file])} for carrier: #{@carrier_name}"]}) if import_sa.failure?

        Success(@plans_file)
      rescue
        return Failure({errors: ["Failed to fetch service area data for carrier: #{@carrier_name}"]}) if input.nil?
        Failure({errors: ["Failed to store data from file #{File.basename(input)} for carrier: #{@carrier_name}"]})
      end
    end
    ...
    def process_plans(input)
      begin
        ...
        return Failure({errors: ["Failed to store data from file #{File.basename(input)} for carrier: #{@carrier_name}"]}) if import_plans.failure?
    ...
    def process_rates(input)
      begin
        ...
        return Failure({errors: ["Failed to store data from file #{File.basename(input)} for carrier: #{@carrier_name}"]}) if rates_result.failure?
