...
RSpec.describe Operations::CountiesLookup, type: :model do
  ...
  it 'should return a success of hash' do
    expect(subject.call({hra_zipcode: countyzip.zip, tenant: 'ma'}).success.class).to eq(Hash)
  end

  it 'should return a success of hash with a specific key' do
    expect(subject.call({hra_zipcode: countyzip.zip, tenant: 'ma'}).success).to have_key(:counties)
  end

  it 'should return counties with matching zipcode' do
    @counties = subject.call({hra_zipcode: countyzip.zip, tenant: 'ma'}).success.values.first
  ...
  it 'should return sorted counties list' do
    expect(subject.call({hra_zipcode: countyzip.zip, tenant: 'ma'}).success.values.first).to eq(['Berkshire', 'Essex'])
  end

  it 'should not return counties with non-matching zipcode' do
    expect(subject.call({hra_zipcode: countyzip.zip, tenant: 'ma'}).success.values.first).not_to include(countyzip3.county_name)
  end

  it 'should not return unsorted counties list' do
    expect(subject.call({hra_zipcode: countyzip.zip, tenant: 'ma'}).success.values.first).not_to eq(['Essex', 'Berkshire'])
  end
end
